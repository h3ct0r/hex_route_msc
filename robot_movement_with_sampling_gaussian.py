...
while is_loop_active:
    ...
    if len(robot_square_array) > 0:
        ...
        if not robot_battery_trajectory:
            # If there is no robot battery trajectory defined
            ...
            # When a robot battery trajectory is defined
            for i in range(len(robot_array)):
                robot = robot_array[i]
                ...
                if len(robot.actual_battery_trajectory) == 0 and  len(robot.battery_trajectory) > 0:
                    robot.actual_battery_trajectory = robot.battery_trajectory.pop(0)

                pygame.draw.circle(screen, RED, robot.waypoint_list[0], 2)
                mov_x, mov_y = calculate_next_movement(robot.actual_pos, robot.waypoint_list[0], 2.5)
                if mov_x == 0 and mov_y == 0:
                    if robot.status == "start":
                        print "continue"
                        robot.status = "continue"
                        robot.remove_waypoint_hexagon_without_counter()
                    elif robot.status == "go_home":
                        robot.remaining_battery = robot_battery
                        robot.status = "start"
                    else:
                        if len(robot.waypoint_list) > 0:
                            robot.remove_waypoint_hexagon_with_counter()
                            if robot.hexagon_counter == robot.hex_max - 1:
                                print "hex reached stop"
                                robot.status = 'reached_stop'
                                robot.actual_battery_trajectory.pop(0)
                                if len(robot.actual_battery_trajectory) == 0:
                                    robot.status = "go_home"
                                    robot.waypoint_list.insert(0, start_point)
                    continue

                calculated_x = robot.actual_pos[0] + mov_x
                calculated_y = robot.actual_pos[1] + mov_y
                robot.update_actual_pos((calculated_x, calculated_y))
    ...
    for robot in robot_square_array:
        ...
        #'''
        for pos in robot.get_previous_pos():
            pygame.draw.line(screen, BLACK, pos, pos, 2)
            pass
        #'''
